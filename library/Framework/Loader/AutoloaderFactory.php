...
{
    const STANDARD_AUTOLOADER = 'Framework\Loader\StandardAutoloader';
     ...
     *
     * @param  array|Traversable $options (optional) options to use. Defaults to Framework\Loader\StandardAutoloader
     * @return void
    ...
    public static function factory($options = null)
    {
        ...
        foreach ($options as $class => $autoloaderOptions) {
            if (!isset(static::$loaders[$class])) {
                ...
                if (!static::isSubclassOf($class, 'Framework\Loader\SplAutoloader')) {
                    throw new \InvalidArgumentException(
                        sprintf('Autoloader class %s must implement Framework\\Loader\\SplAutoloader', $class)
                    );
                }
     ...
     * allow resolving autoloaders not under the "Framework" namespace.
     *
     * @return SplAutoloader
     */
