<?php
/**
 * Zend Spark (http://framework.zend.com/)
 *
...
{
    const STANDARD_AUTOLOADER = 'Spark\Loader\StandardAutoloader';
     ...
     *
     * @param  array|Traversable $options (optional) options to use. Defaults to Spark\Loader\StandardAutoloader
     * @return void
    ...
    public static function factory($options = null)
    {
        ...
        foreach ($options as $class => $autoloaderOptions) {
            if (!isset(static::$loaders[$class])) {
                ...
                if (!static::isSubclassOf($class, 'Spark\Loader\SplAutoloader')) {
                    throw new \InvalidArgumentException(
                        sprintf('Autoloader class %s must implement Spark\\Loader\\SplAutoloader', $class)
                    );
                }
     ...
     * allow resolving autoloaders not under the "Spark" namespace.
     *
     * @return SplAutoloader
     */
