...
class StandardAutoloader implements SplAutoloader
{
    ...
    const AUTOREGISTER_FRAMEWORK  = 'autoregister_framework';
    ...
    public function setOptions($options)
    {
        ...
        foreach ($options as $type => $pairs) {
            switch ($type) {
                case self::AUTOREGISTER_FRAMEWORK:
                    if ($pairs) {
                        $this->registerNamespace('Spark', dirname(__DIR__));
                    }
                    break;
                case self::LOAD_NS:
