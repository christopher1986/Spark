...
{    
     ...
     */
    public function __construct(\wpdb $wpdb, $query)
    {
    ...
    public function fetch($type = self::FETCH_OBJ, $rowOffset = 0)
    {
        $results = array();
        if ($this->isPrepared) {
            $results = $this->connection->get_row($this->preparedQuery, $type, $rowOffset);
        }
    ...
    public function fetchAll($type = self::FETCH_OBJ)
    {
        $results = array();
        if ($this->isPrepared) {
            $results = $this->connection->get_results($this->preparedQuery, $type);
        }
    ...
    public function fetchColumn($columnOffset = 0)
    {
        $results = array();    
        if ($this->isPrepared) {
            $results = $this->connection->get_col($this->preparedQuery, $columnOffset);
        }
    ...
    {
        return $this->connection->rows_affected;
    }
    ...
    private function prepareQuery($query, array $params)
    {
        ...
        foreach ($params as $param) {
            // apply a similar technique as the wpdb class.
            if ($param->getType() === Parameter::PARAM_STR) {
                $query = str_replace("'{$param->getName()}'", "{$param->getName()}", $query);
                $query = preg_replace("/(?!\")(?<!\"){$param->getName()}/", "\"{$param->getName()}\"", $query);
            }
            
            $value = $param->getValue();
            if (is_array($value)) {
                array_walk($value, array($this->connection, 'escape_by_ref'));
                if ($param->getType() === Parameter::PARAM_STR) {
                    $value = implode('", "', $value);
                } else {
                    $value = implode(',', $value);
                ...
                $this->connection->escape_by_ref($value);
            }
